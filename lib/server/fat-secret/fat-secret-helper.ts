...
function makeApiCall(
  ...
  return fetch(`${API_PATH}?${queryString.stringify(queryParams)}`, {
    method: httpMethod,
  });
}

export async function searchFoodV2(
  query: string,
  maxResults = 50,
  page_number = 0
) {
  ...
  const data = (await response.json()) as FatSecretFoodSearchV2;
...
export async function findIdForBarcode(barcode: string) {
  ...
  const response = await makeApiCall(methodParams);
  return response.json() as any as FatSecretBarcodeResponse;
}
...
export async function autocomplete(
  ...
  const response = await makeApiCall(methodParams);
  return response.json() as any as FatSecretAutocompleteResponse;
}
