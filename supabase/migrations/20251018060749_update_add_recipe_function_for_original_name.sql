-- Update the add_recipe function to handle original_name field for ingredients
    ...
    if ingredients is not null and array_length(ingredients,1) > 0 then
      ...
      LOOP
        if ingredients[i]::json->>'type' = 'header' then
          ...
          -- Now including original_name field
          insert into ingredient(recipe_id, user_id, "order", type, meta, number_of_servings, food_id, serving_id, original_name)
          values (add_recipe.recipe_id, user_id, i, 'ingredient', ingredients[i]::json->>'meta', cast(ingredients[i]::json->>'number_of_servings' as double precision), food_uuid, serving_uuid, ingredients[i]::json->>'original_name');
        end if;
      END LOOP;
    end if;
